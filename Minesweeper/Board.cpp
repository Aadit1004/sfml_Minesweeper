...
Board::Board(int t_numRows, int t_numCols, int t_numMines) : m_numRows(t_numRows), m_numCols(t_numCols), m_numMines(t_numMines), m_isGameEnded(false) {}

void Board::generateBoard(sf::RenderWindow* t_windowRef) {
    float tileSize;
    if (m_numMines == 10) {
        tileSize = 50.0f;
    } else if (m_numCols == 40) {
        tileSize = 45.0f;
    } else {
        tileSize = 26.0f;
    }
    ...
    m_grid.resize(m_numRows, std::vector<Tile>(m_numCols, Tile(false, { tileSize, tileSize }, { 0.0f, 0.0f })));

    for (int row = 0; row < m_numRows; ++row) {
        for (int col = 0; col < m_numCols; ++col) {
            float x = startX + col * tileSize;
            float y = startY + row * tileSize;
            m_grid[row][col] = Tile(false, { tileSize, tileSize }, { x, y });
        }
    }
